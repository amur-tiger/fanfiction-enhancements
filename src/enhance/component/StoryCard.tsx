...
export class StoryCard implements Component {
	...
	private addHeader(element: HTMLDivElement, story: Story): void {
		const header = <div class="ffe-sc-header">
			<Rating rating={story.rating}/>
			<a href={"/s/" + story.id} class="ffe-sc-title">{story.title}</a>
	...
	private addTags(element: HTMLDivElement, story: Story) {
		...
		if (story.reviews) {
			tags.appendChild(<span class="ffe-sc-tag ffe-sc-tag-reviews">
	...
	private addFooter(element: HTMLDivElement, story: Story) {
		...
		if (story.words) {
			const words = <div style="float: right;">
		...
		if (story.published) {
			const published = <span class="ffe-sc-footer-info">
				<b>Published:</b>&nbsp;
		...
		if (story.updated) {
			const updated = <span class="ffe-sc-footer-info">
				<b>Updated:</b>&nbsp;
